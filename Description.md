
﻿
• Intermediary Deadline (09.04.2025 23:59): 
To be included at the root of your
 project’s repository:

– Project Description (Description.md): A Markdown document describing:
 ∗ The context and basic narrative of your game./
 ∗ Player characters./
 ∗ Stages (initial ideas for unique effects and restrictions for the connections).
 ∗ Items./
 ∗ Interactions (commands)./
 ∗ Enemy types.
 ∗ Point systems (health, gold, etc.).

– Task Distribution: as issues on GitHub.

# Project Description 
## Context & Basic Narrative
### Context
 In a world where fantastic creatures are commonplace, mankind must fight terrifying monsters to survive.
Among all the kingdoms in existence, the Kingdom of Midland is one of the most powerful, with the Imperial Order and a number of powerful individuals acting on their behalf, known as Heros.
### History
The protagonist is one of these Heroes, commissioned by the Imperial Order to investigate a certain dungeon, the Karadûm's Keep, following numerous attacks in the region, to eliminate the monsters. Little does the protagonist know that he will find something more than a problem for the Midland's Kingdom.

## Player Characters
|   Class     | Health Point |Mana|Stamina| Attack| Speed | Defense | Magic
|-----------------|-----------------|-----------------|-----|---|---|--|--|
| Knight| 170 |10|35|40|8|30|5
|Paladin | 150 |25|30|30|12|25|25
| Hunter | 110 |15|25| 50|20|15|15
| Druid| 120 |30|20|20|10|15|40
| Pyromancer| 90 |35|15|10|15|5|60

### Special Skills

- Knight : Knight's will (+25% DEF, +25% ATK for 2 turns)
- Paladin : Divine Lightning (Light damage + healing 15 HP)
- Hunter : Piercing Arrow (ignores 50% enemy DEF)
- Pyromancer : Supreme fireball (burns for 3 turns)
- Druid : Chastisement of the Ancients (Damage + life theft 10%)

## Affinity System

|   Affinty     | Strong against |Weak against| special effects | 
|-----------------|-----------------|-----------------|-----|
| Fire| Ice |Water|2-turn burn|
|Ice | Water |Fire|Slowing down (25%)|
| Light | Darkness |Darkness|Immunity to curses|
| Darkness| Light |Light|life theft (+5% per attack)|
| Earth| Fire |Water|armor +20|
| Water| Fire |Ice|Care +10% in aquatic zone|

## Stages / Map
-->Map.png

## Objects 
Equipment:
1. Weapons
	Sword of the Crusader (Knight): +20% damage against undead

	Bow of the Hunter (Hunter): +25% damage

	Natural Staff (Druid): Healing +10% at night

	Wand of Fire (Pyromancer): Burn (3 dmg/turn for 2 turns)

	Sacred Hammer (Paladin): Heals 5% of health after a hit

2. Armor
	Plate armor: +15 DEF

	Druid tunic: Regenerates 2 MP/turn

	Shadow cloak (Hunter): +10% dodge

	Flaming robe (Pyromancer): Reduces ice damage by 30%.

3. Consumables
	Healing Potion: +50 HP

	Mana Potion: +20 MP

4. Special items
Dragon Soul Fragment 
Ring of Eternal Ice
Primitive Mace
Secret Scroll
Elixir of Honor

## Interactions (commands)
Assess: reveals information about the target, e.g., their skill set, their statistics, and so on.

Interact: search for / activate a mechanism

Status : Display current health, inventory, and statistics.

Map : Show the stage layout and connections.

Quit : Quit the game.

stage [ID] - Display information about a specific stage

help - List all available commands

## Enemy & Behaviors

### Regular Enemies

| Enemy         | HP  | ATK | DEF | SPD | Affinity | Skills                        |
|---------------|-----|-----|-----|-----|----------|------------------------------------|
| 🧌 Goblin      | 60  | 15  | 5   | 18  | Earth    | May steal items, flee at low HP   |
| 💀 Skeleton    | 80  | 20  | 15  | 10  | Darkness | Revives once, magic resistant      |
| 🧟 Undead      | 100 | 18  | 10  | 8   | Darkness | Poison, heals over time            |
| 🐺 Werewolf    | 120 | 25  | 12  | 22  | None     | Stronger at night                  |
| 👻 Ice Specter | 90  | 15  | 8   | 20  | Ice      | Can freeze, drains MP              |

---

### Boss Enemies

| Boss                          | HP  | ATK | DEF | SPD | Affinity | Skills                      |
|-------------------------------|-----|-----|-----|-----|----------|-------------------------------------|
| 🗿 Cave Giant                 | 250 | 30  | 30  | 5   | Earth    | AoE damage, breaks armor            |
| 🦴 Necromancer                | 200 | 20  | 15  | 10  | Darkness | Summons undead, drains HP/MP        |
| 🧪 Mad Alchemist *(optional)* | 180 | ??  | 10  | 14  | None     | Random potion effects               |
| 🐉 Valdrakar                 | 400 | 35  | 25  | 16  | Darkness | Gets stronger <50% HP, AoE breath   |

## Point Systems

Health Point

Mana Point

Stamina

Experience (lvl 1 – lvl4)

  

### Bonus Features 

2D art with ASCII Art 
Easter Egg
